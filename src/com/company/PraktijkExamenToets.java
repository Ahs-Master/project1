...
public class PraktijkExamenToets extends ToetsVragen {
    ...
    public static void ToetsMakken(){  // om de Toets te beginnen
        ...
        for (int vraag = 0; vraag < praktijikVragen.size(); vraag++) {   // de vragen uitprinten op scherm.
            System.out.println(praktijikVragen.get(vraag).getVraag());

            //om te checken hoeveel keuzes je heb en keuzes op de scherm uitprinten
            int numKeuzes = praktijikVragen.get(vraag).getKeuzes().size();
            for (int keuze = 0; keuze < numKeuzes; keuze++) {
                System.out.println((keuze + 1) + ": " + praktijikVragen.get(vraag).getKeuzes().get(keuze));
            }
            ...
            System.out.println();
            ArrayList<String> Keuzes = praktijikVragen.get(vraag).getKeuzes();
        ...
        scanner.close();
        System.out.printf("je heb : %s van de %s punten gehaald. ",correcteAantwoorden,praktijikVragen.size());
    }
    public void vragenMaken() {
    ...
    public static void displayVragen(){
        ...
        for (int i = 0; i < praktijikVragen.size(); i++){
            System.out.println(praktijikVragen.get(i).getVraag());
            System.out.println();
        }
    }
}