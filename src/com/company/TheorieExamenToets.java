...
public class TheorieExamenToets extends ToetsVragen {
    ...
    public static void ToetsMakken() {
        ...
        for (int vraag = 0; vraag < theorieVragen.size(); vraag++) {   // de vragen uitprinten op scherm.
            System.out.printf("Vraag %d: %s%n", vraag + 1, Vragen.get(vraag));


            int numKeuzes = theorieVragen.get(vraag).getKeuzes().size();
            for (int keuze = 0; keuze < numKeuzes; keuze++) {
                System.out.println((keuze + 1) + ": " + theorieVragen.get(vraag).getKeuzes().get(keuze));
            }
            ...
            ArrayList<String> Keuzes = theorieVragen.get(vraag).getKeuzes();
        ...
        scanner.close();
        System.out.printf("je heb : %s van de %s punten gehaald. ", correcteAantwoorden, theorieVragen.size());
    }
    ...
    //<------Om de vragen op scherm te tonen------->
    public static void displayVragen() {
        vragenMaken();
        for (int i = 0; i < theorieVragen.size(); i++) {
            System.out.printf("Vraag %d: %s%n", i + 1, theorieVragen.get(i).getVraag(i));
            System.out.println();
        }
    ...
    public static void displayKeuzes() {
        vragenMaken();
        for (ToetsVragen toetsVragen : theorieVragen) {
